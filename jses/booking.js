...
async function collectData(){
    const mex =  fetch('http://localhost:5000/api/main/getAll').then(res => res.json())// code to fetch
    ...
    let busListDiv = document.querySelector(".busList");
    let AlChidren = document.querySelectorAll('.busCard');
    
    let MainCount = 0;
    for (let i=0; i<datum.length;i++){
    
        if (MainCount < datum.length && AlChidren.length < datum.length){
            busListDiv.innerHTML += `
            <div class="busCard busId${MainCount}">
            ...
            <div class="BusInfo">
            <p>${datum[MainCount].busArray}</p>
            <p><span class="Ac">A/C</span> ${datum[MainCount].busDepartureState} to ${datum[MainCount].busDestinationState}-${datum[MainCount].busDestinationLGA}  </p>
            <p class="availableSeats"> (${datum[MainCount].busAvailableSeats} seats available) </p>
            <p class="busPrice"> Adult: <span class="AdultPrice"> ${datum[MainCount].adultPrice} </span> Child: <span class="ChildPrice"> ${datum[MainCount].childPrice} </span></p>
            </div>
    
            <div class="busFareDiv">
            <p class="deparTime"> ${datum[MainCount].departureTime} </p>
            <p class="BusFare"> BusFare: 26,000</p>
            <button class="payFare" type="button"><a href="paymentPage/payment.html" class="payFareLink"> Pay Fare</a></button>
            </div>
    ...
    async function ModifyChoices() {
        ...
        for(let DivCounter = 0; DivCounter < AllChildren.length; DivCounter++){
            console.log("divcounter: ", datum[DivCounter], DivCounter, AllChildren.length, datum.length)
            if ( datum[DivCounter].busDestinationState != destinationValue || datum[DivCounter].busDepartureState != departureValue || seatSelected > datum[DivCounter].busAvailableSeats){
                AllChildren[DivCounter].style.display = "none";
            }
            else{
                AllChildren[DivCounter].style.display = "grid";
